...
Leave IMAGES and BRANCHES below empty to get the latest sources.

[source, sh]
----
$ export IMAGES="--images 'openshift/origin-${component}:v0.4.4'"
$ export BRANCHES="--branch v0.4.4"
...
$ vagrant vagrant checkout-repos $BRANCH
...
$ vagrant build-openshift --images
$ vagrant build-sti --binary-only
$ vagrant bootstrap-openshift --with-registry-proxy $IMAGES
